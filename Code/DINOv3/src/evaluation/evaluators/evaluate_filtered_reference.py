"""Filtered-reference evaluation on the Netherlands F3 field dataset.

Evaluates Image Impeccable-trained checkpoints on F3 using a dip-steered
median-filtered volume as a pseudo-clean reference. Reports both standard
no-reference diagnostics and pseudo-paired agreement metrics against the
filtered reference.

IMPORTANT: The filtered reference is a pseudo-clean teacher output, not clean
ground truth. Report pseudo-paired metrics as "filtered-reference agreement",
never as "real field ground-truth accuracy".

Usage:
    python evaluation/evaluate_filtered_reference.py \
        --config configs/<config>_daic.yaml \
        --checkpoint /path/to/best.pt \
        --data-root /path/to/Dataset/F3 \
        --out-dir experiments/runs/robustness/f3_filtered_ref/2d/.../seed42_run01 \
        [--ref-npy /path/to/f3_filtered_ref.npy] \
...
from data.filtered_reference import FilteredReferenceDataset
from evaluation.common.metrics import compute_ms_ssim, compute_ms_ssim_r, compute_mse, compute_psnr
...
# ---------------------------------------------------------------------------
# No-reference diagnostics (same as evaluate_robustness.py)
# ---------------------------------------------------------------------------
...
def _noref_metrics(denoised: torch.Tensor, noisy: torch.Tensor) -> dict:
...
def _filtered_ref_metrics(denoised: torch.Tensor, ref: torch.Tensor, noisy: torch.Tensor) -> dict:
    """Pseudo-paired metrics vs filtered reference plus no-reference diagnostics."""
    noref = _noref_metrics(denoised, noisy)
    paired = {
        # Agreement with the filtered reference (NOT ground-truth accuracy).
        "ms_ssim_ref": compute_ms_ssim(denoised, ref),
        "mse_ref": compute_mse(denoised, ref),
        "psnr_ref": compute_psnr(denoised, ref),
    }
    return {**noref, **paired}
...
def _save_panel(
    ...
    denoised: np.ndarray,
    filtered_ref: np.ndarray,
    path: Path,
) -> None:
    _, axes = plt.subplots(1, 3, figsize=(12, 4))
    panels = [
        ("Noisy input", noisy_center),
        ("Denoised output", denoised),
        ("Filtered reference", filtered_ref),
    ]
    for ax, (title, img) in zip(axes, panels):
        ...
        ax.axis("off")
    plt.suptitle(
        "F3 Filtered-Reference Evaluation\n"
        "Reference = dip-steered median filter (pseudo-clean, not ground truth)",
        fontsize=9,
        style="italic",
    )
    plt.tight_layout()
...
def run_filtered_ref(
    ...
    ref_meta_path = data_root / "processed" / "f3_filtered_ref_meta.json"

    if not npy_path.exists():
        sys.exit(
            f"F3 noisy volume not found at {npy_path}\n"
            "Run: python data/prepare_f3.py --input <f3.segy> --output <data_root>/processed/"
        )
    if not ref_npy.exists():
        sys.exit(
            f"Filtered reference not found at {ref_npy}\n"
            "Run: python data/prepare_filtered_ref.py --input <filtered.segy> "
            "--reference-npy <f3_original.npy> --output <data_root>/processed/"
        )

    mode = str(cfg["data"]["mode"])
    neighbor_stride = int(cfg["data"].get("neighbor_stride", 1))

    ds = FilteredReferenceDataset(
        npy_path=npy_path,
        ref_npy_path=ref_npy,
        mode=mode,
        ...
        ref_meta_path=ref_meta_path if ref_meta_path.exists() else None,
    ...
    print(f"\n--- F3 filtered-reference evaluation ({orientation}) ---")
    with torch.no_grad():
        for noisy_batch, ref_batch, meta_batch in loader:
            ...
            for i in range(noisy_batch.shape[0]):
                ...
                ref_i = ref_batch[i:i + 1]
                metrics = _filtered_ref_metrics(pred_i, ref_i, noisy_i)
                ...
                if sample_counter in panel_indices:
                    panel_batch_items[sample_counter] = (noisy_i, pred_i, ref_i, meta_i)
                sample_counter += 1

    in_chans = int(cfg["model"].get("in_chans", 3))
    center_channel = in_chans // 2
    for sample_idx, (noisy_i, pred_i, ref_i, meta_i) in panel_batch_items.items():
        panel_name = f"f3_filt_panel_{meta_i.get('section_global_id', sample_idx)}.png"
        _save_panel(
    ...
    if rows:
        ...
        csv_path = out_dir / "f3_filtered_ref_metrics.csv"
        ...
        print("  --- Pseudo-paired (vs filtered reference) ---")
        for key in ["ms_ssim_ref", "mse_ref", "psnr_ref"]:
            vals = [r[key] for r in rows if isinstance(r.get(key), (int, float))]
            if vals:
                print(f"  {key}: {np.mean(vals):.4f} +/- {np.std(vals):.4f}")
        print("  --- No-reference diagnostics ---")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Filtered-reference evaluation on F3 using dip-steered median filter."
    )
    ...
    parser.add_argument("--data-root", required=True, help="Root of F3 dataset (contains processed/).")
    parser.add_argument(
        "--ref-npy",
        default=None,
        help=(
            "Path to f3_filtered_ref.npy. "
            "Defaults to <data-root>/processed/f3_filtered_ref.npy."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=32,
        help="Sections per orientation to sample, or 'all' (default: 32).",
    )
    parser.add_argument(
        "--common-context-radius",
        type=int,
        default=None,
        help="Shared valid-center radius. Use 2 to evaluate 2D/3ch/5ch on the same section IDs.",
    )
    ...
    ref_npy = (
        Path(args.ref_npy).resolve()
        if args.ref_npy is not None
        else data_root / "processed" / "f3_filtered_ref.npy"
    )
    ...
    print("\nFiltered-reference evaluation complete.")
