#!/bin/bash
# Vertical-slice training control: 3 variants × 3 training seeds × 3 data seeds = 27 jobs.
#
# Identical to main_multidata/submit_main_multidata.sh in every respect EXCEPT:
#   - data.slice_orientation: vertical  (train + eval on inline/crossline sections
#     instead of the default horizontal time slices)
#   - a separate output study dir (experiments/runs/vertical_orientation/...) so
#     the horizontal main_multidata results are never touched.
# This isolates the effect of TRAINING orientation on the 2D vs 2.5D comparison,
# evaluated against clean Image Impeccable ground truth.
#
...
#
# Submit all 27 jobs:
#   sbatch ~/RP/Code/DAIC/vertical/submit_vertical_main.sh
# Smoke-test a single job first (recommended before the full array):
#   sbatch --array=0 ~/RP/Code/DAIC/vertical/submit_vertical_main.sh
# Resubmit a range:
#   sbatch --array=<A>-<B> ~/RP/Code/DAIC/vertical/submit_vertical_main.sh

#SBATCH --job-name=vert_md_%a
...
#SBATCH --output=/home/nfs/pvarelabernal/RP/Code/DAIC/vertical/slurm_%A_%a.out
#SBATCH --error=/home/nfs/pvarelabernal/RP/Code/DAIC/vertical/slurm_%A_%a.err
...
case "$VARIANT" in
  2d)
    MODE="2d"
    FAMILY="2d"
    RUN_BASE="impeccable_repeated_stride5_lora_r16"
    MODEL_IN_CHANS_LINE=""
    ;;
  3ch)
    MODE="2.5d_3ch"
    FAMILY="3ch"
    RUN_BASE="impeccable_neighbors3_stride5_lora_r16"
    MODEL_IN_CHANS_LINE=""
    ;;
  5ch)
    MODE="2.5d_5ch"
    FAMILY="5ch"
    RUN_BASE="impeccable_neighbors5_stride5_patch_emb_lora_r16"
...
EXP_DIR="$STUDENT_DIR/experiments/runs/vertical_orientation/$FAMILY/$RUN_BASE/data_seed${DATA_SEED}/$RUN_ID"
...
echo "VARIANT: $VARIANT  DATA_SEED: $DATA_SEED  TRAINING_SEED: $TSEED  ORIENTATION: vertical"
