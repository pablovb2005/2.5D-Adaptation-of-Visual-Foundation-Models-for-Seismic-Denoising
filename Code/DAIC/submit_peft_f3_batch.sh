#!/bin/bash
# Batch F3 robustness evaluation for all main_multidata PEFT runs (27 total).
...
#
# Submit:
#   sbatch ~/RP/Code/DAIC/submit_peft_f3_batch.sh

#SBATCH --job-name=peft_f3_batch
...
#SBATCH --output=/home/nfs/pvarelabernal/RP/Code/DAIC/slurm_peft_f3batch_%j.out
#SBATCH --error=/home/nfs/pvarelabernal/RP/Code/DAIC/slurm_peft_f3batch_%j.err
...
EXP_ROOT="$STUDENT_DIR/experiments/runs/main_multidata"
ROB_ROOT="$STUDENT_DIR/experiments/runs/robustness/f3_allsections/main_multidata"

DATA_SEEDS=(101 202 303)
TRAINING_SEEDS=(42 43 44)
VARIANTS=(2d 3ch 5ch)

echo "PEFT F3 batch eval started on $(hostname) at $(date)"
echo "SLURM_JOB_ID: $SLURM_JOB_ID"
echo "EXP_ROOT:     $EXP_ROOT"
echo "ROB_ROOT:     $ROB_ROOT"

if [ ! -x "$PY310" ]; then
    echo "ERROR: Python 3.10 interpreter not found: $PY310" >&2
    exit 127
fi
...
VENV=/tmp/dinov3_py310_peftf3_${SLURM_JOB_ID}
TMPDIR=/tmp/dinov3_py310_peftf3_tmp_${SLURM_JOB_ID}
mkdir -p "$TMPDIR"
...
run_one_eval() {
    ...
    case "$variant" in
        2d)  family=2d;  run_base=impeccable_repeated_stride5_lora_r16 ;;
        3ch) family=3ch; run_base=impeccable_neighbors3_stride5_lora_r16 ;;
        5ch) family=5ch; run_base=impeccable_neighbors5_stride5_patch_emb_lora_r16 ;;
    ...
    local config="$exp_dir/config.yaml"
...
echo ""
echo "=== Starting 27 F3 evaluations ==="
